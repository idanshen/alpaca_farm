...
def stable_resize_token_embeddings_and_tokenizer(
    ...
    stable_resize_token_embeddings(model, len(tokenizer), checkpoint_dir=checkpoint_dir, train_embedding=train_embedding)


def stable_resize_token_embeddings(
    model: transformers.PreTrainedModel, target_size: int, checkpoint_dir: str = None, jitter_new_embeddings=False, train_embedding=True,
):
